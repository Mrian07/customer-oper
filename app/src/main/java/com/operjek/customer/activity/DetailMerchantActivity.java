...
import com.operjek.customer.item.CatItemItem;
import com.operjek.customer.item.ItemItem;
...
import com.operjek.customer.models.CatItemModel;
import com.operjek.customer.models.ItemModel;
import com.operjek.customer.models.PesananMerchant;
...
import com.operjek.customer.utils.api.MapDirectionAPI;
...
public class DetailMerchantActivity extends AppCompatActivity implements ItemItem.OnCalculatePrice {
    ...
    String id, resume, alamat, tutupki, lat, lon, idresto, alamatresto, namamerchant;
    ...
    private RealmResults<PesananMerchant> existingItemMenu;
    private ItemItem ItemMenuItem;
    private Realm realm;
    ...
    private void getdata(Location location) {
        ...
        userService.merchantbyid(param).enqueue(new Callback<MerchantByIdResponseJson>() {
            ...
            public void onResponse(Call<MerchantByIdResponseJson> call, Response<MerchantByIdResponseJson> response) {
                if (response.isSuccessful()) {
                    if (response.body().getMessage().equalsIgnoreCase("success")) {
                        ...
                        hours.setText("Open Hours: " + response.body().getBukamerchant() + "-" + response.body().getTutupmerchant());
                        ...
                        catMerchantNearItem = new CatItemItem(DetailMerchantActivity.this, response.body().getKategori(), R.layout.item_cat_merchant, new CatItemItem.OnItemClickListener() {
                            @Override
                            public void onItemClick(CatItemModel item) {
                                //clicknear.clear();
            ...
            @Override
            public void onFailure(Call<MerchantByIdResponseJson> call, Throwable t) {
    ...
    private void LoadItem() {
        itemAdapter.clear();
        existingItemMenu = realm.where(PesananMerchant.class).findAll();

        for (int e = 0; e < existingItemMenu.size(); e++) {
            existingItemMenu.get(e).getIdItem();
            existingItemMenu.get(e).getQty();
        }

        int[] exiF = new int[itemRealmResults.size()];

        for (int i = 0; i < itemRealmResults.size(); i++) {
            ItemMenuItem = new ItemItem(DetailMerchantActivity.this, this);

            ItemMenuItem.quantity = 0;
            for (int j = 0; j < existingItemMenu.size(); j++) {
                if (existingItemMenu.get(j).getIdItem() == itemRealmResults.get(i).getId_item()) {
                    ItemMenuItem.quantity = existingItemMenu.get(j).getQty();
                    ItemMenuItem.catatan = existingItemMenu.get(j).getCatatan();
                    exiF[i] = existingItemMenu.get(j).getQty();
                }
            }
    ...
    public void onBackPressed() {
        ...
        for (int p = 0; p < existingItem.size(); p++) {
            quantity += existingItem.get(p).getQty();
        }
    ...
    private void sheetlist(int position) {
        ...
        final View mDialog = getLayoutInflater().inflate(R.layout.sheet_detail_item, null);
        ///////////
        TextView text = (TextView) mDialog.findViewById(R.id.title);
        ImageView imageView = (ImageView) mDialog.findViewById(R.id.imageview);
        WebView content = (WebView) mDialog.findViewById(R.id.content);
        TextView kategori = (TextView) mDialog.findViewById(R.id.kategori);
        TextView hargapromo = (TextView) mDialog.findViewById(R.id.hargapromo);
        TextView harga = (TextView) mDialog.findViewById(R.id.harga);

        if (!selectedItem.getFoto_item().isEmpty()) {
            PicassoTrustAll.getInstance(this)
                    .load(Constants.IMAGESITEM + selectedItem.getFoto_item())
                    .resize(250, 250)
                    .into(imageView);
        }
